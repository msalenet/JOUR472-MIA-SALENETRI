...
```{r}
mingo_buyer_totals <- buyer_details(county = "Mingo", state = "WV", key = key)

mingo_pop <- county_population(county = "Mingo", state = "WV", key = key)

mingo_pharma <- total_pharmacies_county(county = "Mingo", state = "WV", key = key)
...
```{r}
rm(mingo_buyer_totals, mingo_pharma, mingo_pop, mingo_raw)
```

```{r}
dc_buyer_totals <- buyer_details(state = "DC", key = key)

dc_pop <- state_population(state = "DC", key = key)

dc_pharma <- total_pharmacies_state(state = "DC", key = key)
```

```{r}
dc_working <- semi_join(dc_pharma, dc_buyer_totals, by = c("buyer_dea_no" = "BUYER_DEA_NO"), copy = FALSE) 
```

```{r}
dc_annual_pharma <- combined_buyer_annual(county = "District of Columbia", key = key)
```

```{r}
dc_working_annual <- inner_join(dc_working, dc_annual_pharma, by = c("buyer_state", "buyer_county", "buyer_dea_no"), copy = FALSE) %>%
  select("buyer_state", "buyer_dea_no", "buyer_name", "year", "dosage_unit")
```

```{r}
dc_annual_pharma <- clean_names(dc_annual_pharma)
```
