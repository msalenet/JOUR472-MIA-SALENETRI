...
logan_pills_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_county == "LOGAN", buyer_state == "WV") %>%
  select(buyer_county, year, dosage_unit)
```

```{r}
#yearly pill totals in logan county
ggplot(logan_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit, fill=buyer_county), fill="forest green") +
  labs(x="Year", y="Total pills", title="Steady flow of opioids in Logan County", subtitle = "Total pills per year shipped to Logan County, West Virginia \nshow very little variation from year to year", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
...
ggplot(WV_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="Some West Virginia counties saw greater opioid increases than others", subtitle = "Some West Virginia counties saw steady increases in the supply \nof opioid pills while some saw spikes in 2008 and 2009", caption = "Source: DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
  facet_wrap(nrow=2, . ~ buyer_county) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
