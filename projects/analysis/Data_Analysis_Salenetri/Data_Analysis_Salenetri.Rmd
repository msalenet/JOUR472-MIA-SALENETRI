...
```{r}
race_data_x %>%
  ggplot(aes(fill = percent_white, geometry=geometry)) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Percent white',title="percent white", subtitle = "", caption = "Source: census") +
  theme(legend.position="right") +
  scale_fill_viridis_c(option = "plasma",labels = comma)
...
```{r}
life_expectancy %>%
  ggplot(aes(fill = avg_expectancy, geometry = geometry)) +
  geom_sf(lwd = 0) +
  theme_map() +
...
big_data <- big_data %>%
  ...
  select(State, County_Name, Population_2010, RUCC_2013, countyfips, avg_pills_year, Pills_per_person, avg_expectancy, med_income)

#unemployment
big_data <- big_data %>%
  inner_join(unemployment_all, by='countyfips', copy=TRUE) %>%
  select(State, County_Name, Population_2010.x, RUCC_2013.x, countyfips, avg_pills_year.x, Pills_per_person.x, avg_expectancy, avg_unemployment, med_income) 
...
#add race
big_data <- big_data %>%
  full_join(race_data_x, by=NULL, copy=FALSE) %>%
  select(State, County_Name, Population_2010, RUCC_2013, countyfips, avg_pills_year, Pills_per_person, avg_expectancy, avg_unemployment, med_income, avg_use_male, avg_use_female, avg_use, percent_white, percent_non_white, geometry)%>%
  mutate(RUCC_2013 = as.numeric(RUCC_2013))
```
...
big_data %>%
  ungroup() %>%
  select(-State, -County_Name, -countyfips, -avg_pills_year, -RUCC_2013, -geometry) %>%
  #cor()
  correlate() %>%
  select(rowname, Pills_per_person)

# P value for one set of variables
cor.test(big_data$avg_expectancy, big_data$Pills_per_person)

# RUCC_ONLY
rucc_lm <- lm(Pills_per_person ~ RUCC_2013, data=big_data)
summary(rucc_lm)

white_lm <- lm(Pills_per_person ~ percent_white, data=big_data)
summary(white_lm)

non_white_lm <- lm(Pills_per_person ~ percent_non_white, data=big_data)
summary(non_white_lm)

income_lm <- lm(Pills_per_person ~ med_income, data=big_data_focused)
summary(income_lm)

# Linear Model (keep adding new variable with a plus sign.  )
first_lm <- lm(Pills_per_person ~ avg_expectancy+avg_unemployment+percent_white+avg_use+avg_use_male+avg_use_female, data=big_data)
summary(first_lm)

second_lm <- lm(Pills_per_person ~ avg_expectancy+avg_unemployment+med_income+percent_white+RUCC_2013+avg_use+avg_use_male+avg_use_female, data=big_data_focused)
summary(second_lm)

plot(Pills_per_person ~ RUCC_2013, data=big_data)
abline(first_lm)

plot(Pills_per_person ~ avg_expectancy+avg_unemployment+med_income+percent_white+avg_use+avg_use_male+avg_use_female, data=big_data)

glimpse(big_data)


# First point: though this is often thought of as a rural problem, the pill shipments were mostly targeted not at the most remote counties, but at RUCC 4,5,6,7. 
# Second point: here are the people who were targeted by this trend: unemployment (higher rates of unemployment), health (life expectancy (shorter lives)), race (pct_white, pct_nonwhite, -- it's mostly white people), poverty (pct_poverty, higher poverty rate), income (median_household_income, lower median household income), opioid_death_rate (higher rate).  AVERAGES for RUCC GROUP.  REGRESSIONS FOR EACH RUCC GROUP
# Here's where I'll report this story: a county in the RUCC2013_5 group, that has high rates of unemp.  Paint me a picture what it looks like there. 
...
finding the optimal county to ship pills
```{r}
big_data_optimal <- big_data_focused %>%
  select(-med_income, -avg_use_male, -avg_use_female, -percent_white, -percent_non_white, -geometry) %>%
  filter(avg_expectancy < 75) %>%
  filter(avg_unemployment > 10) %>%
  filter(avg_use < 7.5) %>%
  filter(Pills_per_person > 40)

big_data_opt2 <- big_data_focused %>%
  filter(Pills_per_person > 70) %>%
  select(-avg_use_male, -avg_use_female, -percent_non_white, -geometry) 
big_data_opt2 <- big_data_opt2 %>%
  inner_join(county_geodata_shifted, by=NULL, copy=FALSE) %>%
  select(-GEOID, -estimate, -NAME, -variable, -moe)

opt_lm <- lm(Pills_per_person ~ avg_expectancy+avg_unemployment+percent_white+avg_use+med_income, data=big_data_opt2)
summary(opt_lm)

ggplot(big_data_opt2, aes(x=avg_expectancy, y=Pills_per_person, shape=RUCC_2013, color=RUCC_2013)) + 
  geom_point() +
  labs(title="title", subtitle = "", caption = "Source: bls, etc?") +
  geom_smooth(method=lm)
ggplot(big_data_opt2, aes(x=avg_unemployment, y=Pills_per_person, shape=RUCC_2013, color=RUCC_2013)) + 
  geom_point() +
  labs(title="title", subtitle = "", caption = "Source: bls, etc?") +
  geom_smooth(method=lm)
ggplot(big_data_opt2, aes(x=avg_use, y=Pills_per_person, shape=RUCC_2013, color=RUCC_2013)) + 
  geom_point() +
  labs(title="title", subtitle = "", caption = "Source: bls, etc?") +
  geom_smooth(method=lm)
ggplot(big_data_opt2, aes(x=med_income, y=Pills_per_person, shape=RUCC_2013, color=RUCC_2013)) + 
  geom_point() +
  labs(title="title", subtitle = "", caption = "Source: bls, etc?") +
  geom_smooth(method=lm)

big_data_opt2 %>%
  ggplot(aes(fill = Pills_per_person, geometry=geometry)) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='PIlls per person',title="Pills per person by US county", subtitle = "", caption = "Source: ARCOS") +
  theme(legend.position="right") +
