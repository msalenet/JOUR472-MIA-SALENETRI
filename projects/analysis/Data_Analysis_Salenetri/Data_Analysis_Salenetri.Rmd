...
big_data %>%
  ungroup() %>%
  select(-State, -County_Name, -countyfips, -avg_pills_year, -RUCC_2013) %>%
  #cor()
  correlate() %>%
...
first_lm <- lm(Pills_per_person ~ avg_expectancy+avg_unemployment+avg_use+med_income+percent_white+percent_non_white, data=big_data)
summary(first_lm)
