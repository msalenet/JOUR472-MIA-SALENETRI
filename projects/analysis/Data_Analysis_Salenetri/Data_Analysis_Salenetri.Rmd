...
```{r}
#start with ARCOS totals by county (county, state, fips, dosage unit)
```

I think I want to have different datsets for each population group. That way, I can do some analysis on a smaller scale.
```{r}
#split USDA dataset into nine smaller ones by RUCC_2013
#join each one to census, arcos
```
