...
LETS LOOK AT HEALTH DATA
```{r}
county_alc_use <- read_csv("health_data/ALC_USE_ANY_2005_2012.csv")
county_alc_use_heavy <- read_csv("health_data/ALC_USE_HEAVY_2005_2012.csv")

county_alc_use <- county_alc_use %>%
  inner_join(rural_urban_working, by = NULL, copy = FALSE)

county_alc_use_heavy <- county_alc_use_heavy %>%
  inner_join(rural_urban_working, by = NULL, copy = FALSE)

alc_use_gender_heavy <- county_alc_use_heavy %>%
  mutate(avg_female = mean(ends_with(Females)))%>%
  select(-`2006 Both Sexes`, -`2007 Both Sexes`, -`2008 Both Sexes`, -`2009 Both Sexes`, -`2010 Both Sexes`, -`2011 Both Sexes`, -`2012 Both Sexes`, -`Percent Change 2005-2012, Both Sexes`)

```

lets look at a graph of how heavy alcohol use compares to these other variables
```{r}
glimpse(alc_use_gender_heavy)
