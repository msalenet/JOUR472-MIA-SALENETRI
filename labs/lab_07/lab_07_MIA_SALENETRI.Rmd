...
```{r}
# It looks like none or very few of the US territories have population data. This will make it very difficult to compare the rate of opioids shipped to territories versus US states. Usually their populations tend to be smaller than those of states, so if we can only use the amount of pills as a point comparison, it will be extremely misleading.
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Population',title="West Virginia, Kentucky, South Carolina had most opioid pills per person", subtitle = "Nevada, Oklahoma and Tennessee had next most", caption = "Source: ARCOS, Washington Post") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Data shows an increase of pills per person in some states from 2006 to 2012", subtitle = "Many states hit the hardest are in the southeastern United States, particularly in Appalachia", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
#Aside from the large amount of pills in Appalachia, I've noticed that north-midwestern states like North and South Dakota had pretty low rates of pills per person in 2009. Also, many western states (with a notable difference being Nevada) seem to have pretty median rates of pills per person relative to the rest of the country.
...
```{r}
maryland_pills_population_county <- pills_population %>%
  filter(statefips == "24") %>%
...
```{r}
#joining md pills pop w geoid

maryland_pills_population_county <- county_geodata_shifted %>%
 inner_join(maryland_pills_population_county, by=c("GEOID" = "countyfips"))

```

```{r}

maryland_pills_population_county %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Allegany, Cecil, Kent counties saw peak opioid pills in 2011 ", subtitle = "All three counties saw a slow increase of pill shipments between 2006 and 2011. \nBy 2012, the numbers began to dwindle, but Kent and Allegany still held the top spots.", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
